...
class EndGameButton extends StatelessWidget {
  ...
  Future<void> _handleEndGame(BuildContext context) async {
    ...
    final List<Map<String, dynamic>> finalStandings = playerList.map((player) {
      return {
        'name': player.name,
        'score': player.score,
    ...
    AppLogger.i("END GAME: Final standings prepared with ${finalStandings.length} players");

    // Step 3: Calculate deletion timestamp (2 hours in the future)
    final deleteAtTimestamp = DateTime.now()
        .add(Duration(hours: 2))
        .millisecondsSinceEpoch;

    AppLogger.i("END GAME: Room will be deleted at timestamp: $deleteAtTimestamp");
